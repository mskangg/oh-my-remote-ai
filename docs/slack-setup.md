...
oh-my-remote-ai의 첫 공개는 Slack-first입니다. 지금은 사용자가 자신의 Slack workspace에 앱을 만들고 `.env.local`을 채운 뒤 `doctor`로 검증하는 흐름을 가장 짧게 만드는 데 집중합니다.
...
기본 direct install target은 `~/.local/bin/rcc`를 기준으로 잡습니다. setup 마지막에는 binary 배치와 PATH onboarding을 처리하는 shell installer script를 함께 안내합니다.
...
- `SLACK_APP_TOKEN`
- `SLACK_ALLOWED_USER_ID` — 봇을 사용할 수 있는 Slack user ID (쉼표로 여러 명 지정 가능, 예: `U123,U456`)
...
복사하기 쉬운 링크:
- GitHub 보기용: `https://github.com/mskangg/oh-my-remote-ai/blob/main/slack/app-manifest.json`
- raw: `https://raw.githubusercontent.com/mskangg/oh-my-remote-ai/main/slack/app-manifest.json`

현재 manifest는 `/cc`, `/cx`, `/gm` 슬래시 커맨드와 필요한 scope를 포함합니다. private channel에서는 테스트 전에 bot을 초대해야 합니다.

## Channel mapping

oh-my-remote-ai에서는 **Slack 채널 하나가 프로젝트 하나를 대표합니다.**
...
4. 채널을 만든 직후에는 **`/invite @oh-my-remote-ai`** 로 bot user를 먼저 초대합니다.
...
추가 운영 체크:
- 테스트 전에 대상 채널에 **`/invite @oh-my-remote-ai`** 를 실행했는지 확인하세요.

1. required Slack env vars 존재 여부 (bot token, app token, signing secret, allowed user IDs)
2. `.env.local` 존재 여부
3. `tmux` 사용 가능 여부
4. 상태 DB 경로 생성 가능 여부
...
8. `codex` 설치 여부 (optional, `/cx` 사용 시 필요)
9. `gemini` 설치 여부 (optional, `/gm` 사용 시 필요)
