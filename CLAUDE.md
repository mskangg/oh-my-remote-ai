...
- `transport-slack`: Slack Socket Mode + API adapter (유스케이스 소유 금지)
  모듈: `types` / `ports` / `formatting` / `store` / `transport` / `publisher` / `socket_mode`
...
- 블랙박스 인수 테스트: `crates/<크레이트>/tests/acceptance.rs` (pub API만 접근, Gherkin 시나리오 주석 + 한글 픽스처 함수)
- `SessionState`에 직접 패턴 매칭 금지 — `display_label()`, `is_in_progress()`, `is_runtime_active()`, `is_idle()`, `is_failed()` 메서드 사용 (Tell Don't Ask)
- 위험 명령어 키워드 추가: `crates/policy/src/lib.rs`의 `APPROVAL_REQUIRED_KEYWORDS` const에만 추가 (함수 수정 불필요)
